...
"""
Created on Mon Oct 26 01:07:23 2020

@author: Tristan

Using StockNews API

This script scrape news data from the Yahoo Financial RSS Feed and 
store them with the sentiment of the headline and the summary. Depending on 
the initialization 1 or 2 files are output as csv. No. 1 is the scraped 
news (optional) and no. 2 is the summary, having the summarized sentiment of 
news for the given date (see options) and the values.


######## ISSUE ##########

This API outputs files. We have two options to fix this
1. we can implement extract_file() to extract info we need
2. I have included the original code of StockNews at the bottom; we can use 
   functions in the file directly if this would not violate honor code 
...
def pull_news(stocks):
    """
    Input: a list of tickers
    e.g. stocks = ['AAPL', 'MSFT', 'NFLX']
    
    This function returns a pandas DataFrame and saves it to data/data.csv by 
    default (see options)
    
    -- Options --
    
    stocks: A list of stocks to check. See http://eoddata.com/symbols.aspx for 
        all symbols available
            
    news_file='news.csv': filename of the saved news
    
    summary_file='data.csv': filename of the saved dataset, including sentiment 
        and value per day and stock
                            
    save_news=True: save the news file or scrape and analyse on the fly for
        recent news
        
    closing_hour=20: Close of the exchange (NASDAQ in this case). News after 
        closing will be taken for next trading day (skips the weekend as well)
        
    closing_minute=0: Same as closing_hour
    
    wt_key=None: Your worldtradingdata.com API Key. Get one here. Not needed 
        if read_rss is called directly.
...
def extract_file():
    """
    This function extracts news, summaries, sentiments, etc. from the file 
    generated from pull_news()
    ...
    print("========================================================")
    print("The first news of", df['stock'][0])
    print("Title:")
    print(df['title'][0])
    print("Summary:")
    print(df['summary'][0])
...
BELOW IS THE ORIGINAL CODE OF StockNews API
BELOW IS THE ORIGINAL CODE OF StockNews API
BELOW IS THE ORIGINAL CODE OF StockNews API
